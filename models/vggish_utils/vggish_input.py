...
def wavfile_to_examples(wav_file):
  ...
  wav_data = crop_zeros(wav_data)
  '''
  if len(wav_data.shape) == 2:
    wav_data = np.mean(wav_data, axis=1, dtype=np.int16)
  if len(wav_data) < 6 * sr:
    wav_data = np.pad(wav_data, pad_width=(0, 6 * sr - len(wav_data)), mode='constant')
  '''
