...
from models.Model_CNN_2D import CNN2D_Model
    ...
    def fit(self, dataset, features_list):
        for param_id, params in enumerate(product(*self.param_grid.values())):
            ...
            for train_index, test_index in self.cv.split(dataset, dataset['Emotion']):
                ...
                KF_x_train = np.expand_dims(KF_x_train, axis=3)
                KF_x_val = np.expand_dims(KF_x_val, axis=3)
                KF_x_test = np.expand_dims(KF_x_test, axis=3)
...
features_list =  {'Features': ['mfcc_img_cnn']}
...
gridSearch = GridSearchCV(cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42), param_grid=param_grid, model_class=CNN2D_Model, model_name="CNN2D")
